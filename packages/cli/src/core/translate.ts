...
export async function start(period: number, logger: Logger) {
...
export async function commit(logger: Logger) {
  ...
  logger.info('3. 添加备注 ...');
  await api.$`git commit -m \"chore(translate): ${period}.md\"`;
...
export async function pr(logger: Logger) {
  ...
  logger.info('2. 发起 PR ...');
  await api.$`gh pr create -w -t \"chore(translate): ${period}.md\" -b 'See Files'`;
